...
module.exports = {
	...
	async run(bot, interaction)
	{
		...
		try
		{
			freechamp = await axios.get(`https://${config.region}.api.riotgames.com/lol/platform/v3/champion-rotations?api_key=${config.token_riot}`);
		}
		catch (error)
		{
			return errBot(error.response?.status, bot, interaction);
		}
		if (freechamp.status != 200)
			return errBot(freechamp.status, bot, interaction);
